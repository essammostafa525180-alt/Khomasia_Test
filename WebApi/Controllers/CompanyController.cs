using Application.CQRS.Company.Commands;
using Application.CQRS.Company.Queries;
...
namespace WebApi.Controllers
{
    ...
    public class CompanyController : ControllerBase
    {
        ...
        [HttpPut("update/{id}")]
        public async Task<IActionResult> Update([FromRoute] int id, [FromBody] UpdateCompanyCommand command)
        {
            if (id != command.Id)
                return BadRequest();
            var result = await _mediator.Send(command);
