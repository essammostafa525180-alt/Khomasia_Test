using Application.CQRS.City.Commands;
using Application.CQRS.City.Queries;
...
namespace WebApi.Controllers
{
    ...
    public class CityController : ControllerBase
    {
        ...
        [HttpPut("update/{id}")]
        public async Task<IActionResult> Update([FromRoute] int id, [FromBody] UpdateCityCommand command)
        {
            if (id != command.Id)
                return BadRequest();
            var result = await _mediator.Send(command);
