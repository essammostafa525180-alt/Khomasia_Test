using Application.CQRS.Country.Commands;
using Application.CQRS.Country.Queries;
...
namespace WebApi.Controllers
{
    ...
    public class CountryController : ControllerBase
    {
        ...
        [HttpPut("update/{id}")]
        public async Task<IActionResult> Update([FromRoute] int id, [FromBody] UpdateCountryCommand command)
        {
            if (id != command.Id)
                return BadRequest();
            var result = await _mediator.Send(command);
