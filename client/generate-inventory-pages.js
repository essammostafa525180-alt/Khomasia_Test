...
// service files for the inventory pages inside pages/inventory/.
// Run:  node generate-inventory-pages.js
const fs = require('fs');
const path = require('path');

const ROOT = __dirname;
const MODELS_DIR = path.join(ROOT, 'src/app/Shared/Model');
const INVENTORY_DIR = path.join(ROOT, 'src/app/pages/inventory');

// ---------------------------------------------------------------------------
// Page definitions
// ---------------------------------------------------------------------------
const PAGES = [
  { page: 'item-card', title: 'Item Card', key: 'InventoryItem', model: 'inventory-item.model', config: 'InventoryItem' },
  { page: 'item-balance', title: 'Item Balance', key: 'InventoryItemLocationBatchSerial', model: '-inventory-item-location-batch-serial.model', config: 'InventoryItemLocationBatchSerial' },
  { page: 'item-stock', title: 'Item Stock', key: 'InventoryItemSerial', model: '-inventory-item-serial.model', config: 'InventoryItemSerial' },
  { page: 'asset-name', title: 'Asset Name', key: 'Asset', model: '-asset.model', config: 'Asset' },
  { page: 'asset-move', title: 'Asset Move', key: 'AssetItemMove', model: '-asset-item-move.model', config: 'AssetItemMove' },
  { page: 'asset-issue-request', title: 'Asset Issue Request', key: 'AssetCountIssue', model: '-asset-count-issue.model', config: 'AssetCountIssue' },
  { page: 'grn', title: 'GRN', key: 'VendorOrderReceive', model: '-vendor-order-receive.model', config: 'VendorOrderReceive' },
  { page: 'grn-quality', title: 'GRN Quality', key: 'VendorOrderQuality', model: '-vendor-order-quality.model', config: 'VendorOrderQuality' },
  { page: 'issue-out', title: 'Issue Out', key: 'InventroyItemRequestWithdraw', model: '-inventroy-item-request-withdraw.model', config: 'InventroyItemRequestWithdraw' },
  { page: 'item-return', title: 'Item Return', key: 'InventoryItemReturn', model: '-inventory-item-return.model', config: 'InventoryItemReturn' },
  { page: 'material-request', title: 'Material Request', key: 'InventroyItemRequestWithdraw', model: '-inventroy-item-request-withdraw.model', config: 'InventroyItemRequestWithdraw' },
  { page: 'stock-count-adjustment', title: 'Stock Count Adjustment', key: 'InventoryStockCount', model: '-inventory-stock-count.model', config: 'InventoryStockCount' },
  { page: 'stock-count-list', title: 'Stock Count List', key: 'InventoryStockCount', model: '-inventory-stock-count.model', config: 'InventoryStockCount' },
  { page: 'supplier-return', title: 'Supplier Return', key: 'VendorReturn', model: '-vendor-return.model', config: 'VendorReturn' },
  { page: 'transfer', title: 'Transfer', key: 'InventoryTransfere', model: '-inventory-transfere.model', config: 'InventoryTransfere' },
];
...
function buildViewTs(page, fields, columns, labelField) {
  ...
  const formClass = `${page.key}FormComponent`;
...
export class View${pascal}Component implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/inventory/${page.page}/new']);
   }
   ...
   onEdit(row: ${page.key}): void {
     ...
     this.router.navigate(['/inventory/${page.page}', row.id, 'edit']);
   }
...
${dateHelper}
  private close(saved: boolean): void {
    ...
    this.router.navigate(['/inventory/${page.page}']);
  }
}
`;
}
...
function buildFormHtml(page, fields) {
  ...
  const bannerIcon = page.key === 'VendorOrderReceive' ? 'assignment' : 'edit';

  return `<div class="page-card" [class.page-card--dialog]="isDialog">
  <div class="banner" *ngIf="!isDialog">
    <div class="banner__icon">
      <mat-icon>{{ mode === 'create' ? 'add_circle' : '${bannerIcon}' }}</mat-icon>
    </div>
...
for (const page of PAGES) {
  ...
  const dir = path.join(INVENTORY_DIR, page.page);
  ...
  console.log(`  ${page.page.padEnd(24)} cols=[${columns.map((c) => c.name).join(', ') || 'id'}] formFields=${fields.length}`);
}

console.log(`Generated ${total} files for ${PAGES.length} pages.`);
