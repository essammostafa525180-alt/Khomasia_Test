...
import { SalesInvoice } from '../../../Shared/Model/-sales-invoice.model';
...
export class ViewSalesInvoiceComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select', 'id', 'customerId', 'userId', 'address', 'contactPerson', 'vatpercentage', 'vatamount', 'actions'];
  ...
  exportColumns: ExportColumn[] = [
    ...
    { key: 'vatpercentage', label: 'Vatpercentage' },
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<SalesInvoice[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/reports/sales-invoice/new']);
   }
   ...
   onEdit(row: SalesInvoice): void {
     ...
     this.router.navigate(['/reports/sales-invoice', row.id, 'edit']);
   }
   ...
   onDelete(row: SalesInvoice): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Sales Invoice', message: 'Delete "' + (row.address ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Sales Invoice removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: SalesInvoice }): void {
    this.dialog
      .open(SalesInvoiceFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
