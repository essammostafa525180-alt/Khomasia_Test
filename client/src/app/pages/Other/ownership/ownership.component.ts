...
import { Ownership } from '../../../Shared/Model/-ownership.model';
...
export class ViewOwnershipComponent implements AfterViewInit {
  ...
  selection = new SelectionModel<Ownership>(true, []);
  ...
  constructor(
    public service: OwnershipService,
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Ownership[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/ownership/new']);
   }

   onEdit(row: Ownership): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/other/ownership', row.id, 'edit']);
   }

   onDelete(row: Ownership): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Ownership', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Ownership removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: Ownership): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: Ownership }): void {
    this.dialog
      .open(OwnershipFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
