...
export class ViewZoneComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Zone[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/zone/new']);
   }
   ...
   onEdit(row: Zone): void {
     ...
     this.router.navigate(['/other/zone', row.id, 'edit']);
   }
   ...
   onDelete(row: Zone): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Zone', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Zone removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Zone }): void {
    this.dialog
      .open(ZoneFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
