...
export class ViewCustomerComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select', 'id', 'code', 'name', 'nameAr', 'phone', 'address', 'contactPerson', 'actions'];
  ...
  ngAfterViewInit(): void {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/customer/new']);
   }
   ...
   onEdit(row: Customer): void {
     ...
     this.router.navigate(['/other/customer', row.id, 'edit']);
   }
   ...
   onDelete(row: Customer): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Customer', message: 'Delete "' + (row.code ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Customer removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Customer }): void {
    this.dialog
      .open(CustomerFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
