...
import { VehicleModel } from '../../../Shared/Model/-vehicle-model.model';
...
export class ViewVehicleModelComponent implements AfterViewInit {
  ...
  constructor(
    public service: VehicleModelService,
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<VehicleModel[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/vehicle-model/new']);
   }
   ...
   onEdit(row: VehicleModel): void {
     ...
     this.router.navigate(['/other/vehicle-model', row.id, 'edit']);
   }
   ...
   onDelete(row: VehicleModel): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Vehicle Model', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Vehicle Model removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: VehicleModel }): void {
    this.dialog
      .open(VehicleModelFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
