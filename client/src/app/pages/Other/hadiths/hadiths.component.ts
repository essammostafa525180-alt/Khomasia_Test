...
import { Hadiths } from '../../../Shared/Model/-hadiths.model';
...
import { HadithsFormComponent } from './hadiths-form.component';
import { ConfirmDialogComponent } from '../../../Shared/Components/confirm-dialog/confirm-dialog.component';
import { MATERIAL_IMPORTS } from '../../../Shared/materail-imports';

@Component({
  selector: 'app-view-hadiths',
  standalone: true,
  imports: [MATERIAL_IMPORTS],
  templateUrl: './hadiths.component.html',
  styleUrl: './hadiths.component.css',
})
export class ViewHadithsComponent implements AfterViewInit {
  ...
  selection = new SelectionModel<Hadiths>(true, []);
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Hadiths[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/hadiths/new']);
   }

   onEdit(row: Hadiths): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/other/hadiths', row.id, 'edit']);
   }

   onDelete(row: Hadiths): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Hadiths', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Hadiths removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: Hadiths): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: Hadiths }): void {
    this.dialog
      .open(HadithsFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
