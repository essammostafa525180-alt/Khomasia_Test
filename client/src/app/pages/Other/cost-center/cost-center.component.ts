...
import { CostCenter } from '../../../Shared/Model/-cost-center.model';
...
export class ViewCostCenterComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<CostCenter[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/cost-center/new']);
   }
   ...
   onEdit(row: CostCenter): void {
     ...
     this.router.navigate(['/other/cost-center', row.id, 'edit']);
   }

   onDelete(row: CostCenter): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Cost Center', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Cost Center removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: CostCenter }): void {
    this.dialog
      .open(CostCenterFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
