...
export class ViewVehicleTypeComponent implements AfterViewInit {
  ...
  selection = new SelectionModel<VehicleType>(true, []);
  ...
  constructor(
    public service: VehicleTypeService,
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<VehicleType[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/vehicle-type/new']);
   }

   onEdit(row: VehicleType): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/other/vehicle-type', row.id, 'edit']);
   }

   onDelete(row: VehicleType): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Vehicle Type', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Vehicle Type removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: VehicleType }): void {
    this.dialog
      .open(VehicleTypeFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
