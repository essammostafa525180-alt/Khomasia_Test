...
import { SubSection } from '../../../Shared/Model/-sub-section.model';
...
@Component({
  selector: 'app-view-sub-section',
  standalone: true,
  imports: [MATERIAL_IMPORTS],
  templateUrl: './sub-section.component.html',
  styleUrl: './sub-section.component.css',
})
export class ViewSubSectionComponent implements AfterViewInit {
  ...
  dataSource = new MatTableDataSource<SubSection>([]);
  selection = new SelectionModel<SubSection>(true, []);
  ...
  constructor(
    public service: SubSectionService,
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<SubSection[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/sub-section/new']);
   }

   onEdit(row: SubSection): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/other/sub-section', row.id, 'edit']);
   }

   onDelete(row: SubSection): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Sub Section', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Sub Section removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: SubSection): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: SubSection }): void {
    this.dialog
      .open(SubSectionFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
