...
import { ServiceType } from '../../../Shared/Model/-service-type.model';
...
export class ViewServiceTypeComponent implements AfterViewInit {
  ...
  selection = new SelectionModel<ServiceType>(true, []);
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<ServiceType[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/service-type/new']);
   }
   ...
   onEdit(row: ServiceType): void {
     ...
     this.router.navigate(['/other/service-type', row.id, 'edit']);
   }
   ...
   onDelete(row: ServiceType): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Service Type', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Service Type removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: ServiceType): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: ServiceType }): void {
    this.dialog
      .open(ServiceTypeFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
