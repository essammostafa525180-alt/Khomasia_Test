...
import { Line } from '../../../Shared/Model/-line.model';
...
export class ViewLineComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Line[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/line/new']);
   }

   onEdit(row: Line): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/other/line', row.id, 'edit']);
   }

   onDelete(row: Line): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Line', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Line removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: Line): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: Line }): void {
    this.dialog
      .open(LineFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
