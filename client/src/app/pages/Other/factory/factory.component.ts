...
import { Factory } from '../../../Shared/Model/-factory.model';
...
export class ViewFactoryComponent implements AfterViewInit {
  ...
  selection = new SelectionModel<Factory>(true, []);

  displayedColumns: string[] = ['select', 'id', 'code', 'description', 'address', 'name', 'nameAr', 'actions'];
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Factory[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/factory/new']);
   }

   onEdit(row: Factory): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/other/factory', row.id, 'edit']);
   }

   onDelete(row: Factory): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Factory', message: 'Delete "' + (row.code ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Factory removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: Factory): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: Factory }): void {
    this.dialog
      .open(FactoryFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
