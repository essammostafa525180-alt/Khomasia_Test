...
export class ViewSectionComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Section[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/section/new']);
   }
   ...
   onEdit(row: Section): void {
     ...
     this.router.navigate(['/other/section', row.id, 'edit']);
   }
   ...
   onDelete(row: Section): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Section', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Section removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: Section): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: Section }): void {
    this.dialog
      .open(SectionFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
