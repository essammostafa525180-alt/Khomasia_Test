...
export class ViewServiceComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Service[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/service/new']);
   }
   ...
   onEdit(row: Service): void {
     ...
     this.router.navigate(['/other/service', row.id, 'edit']);
   }

   onDelete(row: Service): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Service', message: 'Delete "' + (row.code ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Service removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Service }): void {
    this.dialog
      .open(ServiceFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
