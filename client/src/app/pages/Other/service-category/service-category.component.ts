...
import { ServiceCategory } from '../../../Shared/Model/-service-category.model';
...
export class ViewServiceCategoryComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<ServiceCategory[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/service-category/new']);
   }
   ...
   onEdit(row: ServiceCategory): void {
     ...
     this.router.navigate(['/other/service-category', row.id, 'edit']);
   }
   ...
   onDelete(row: ServiceCategory): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Service Category', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Service Category removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: ServiceCategory }): void {
    this.dialog
      .open(ServiceCategoryFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
