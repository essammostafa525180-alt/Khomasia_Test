...
export class ViewVisitComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select', 'id', 'customerId', 'userId', 'latitude', 'longitude', 'image', 'otherSupplier', 'actions'];
  ...
  exportColumns: ExportColumn[] = [
    ...
    { key: 'otherSupplier', label: 'Other Supplier' },
  ];
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Visit[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/visit/new']);
   }

   onEdit(row: Visit): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/other/visit', row.id, 'edit']);
   }

   onDelete(row: Visit): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Visit', message: 'Delete "' + (row.image ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Visit removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Visit }): void {
    this.dialog
      .open(VisitFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
