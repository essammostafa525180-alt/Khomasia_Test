...
export class ViewProjectComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select', 'id', 'code', 'name', 'nameAr', 'actions'];
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Project[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/project/new']);
   }
   ...
   onEdit(row: Project): void {
     ...
     this.router.navigate(['/other/project', row.id, 'edit']);
   }
   ...
   onDelete(row: Project): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Project', message: 'Delete "' + (row.code ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Project removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Project }): void {
    this.dialog
      .open(ProjectFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
