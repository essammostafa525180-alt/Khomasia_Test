...
export class ViewExpenseComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Expense[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/other/expense/new']);
   }
   ...
   onEdit(row: Expense): void {
     ...
     this.router.navigate(['/other/expense', row.id, 'edit']);
   }
   ...
   onDelete(row: Expense): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Expense', message: 'Delete "' + (row.code ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Expense removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Expense }): void {
    this.dialog
      .open(ExpenseFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
