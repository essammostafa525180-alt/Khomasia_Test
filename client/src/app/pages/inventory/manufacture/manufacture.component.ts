...
import { Manufacture } from '../../../Shared/Model/-manufacture.model';
...
export class ViewManufactureComponent implements AfterViewInit {
  ...
  constructor(
    public service: ManufactureService,
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Manufacture[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/inventory/manufacture/new']);
   }

   onEdit(row: Manufacture): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/inventory/manufacture', row.id, 'edit']);
   }

   onDelete(row: Manufacture): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Manufacture', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Manufacture removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: Manufacture): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: Manufacture }): void {
    this.dialog
      .open(ManufactureFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
