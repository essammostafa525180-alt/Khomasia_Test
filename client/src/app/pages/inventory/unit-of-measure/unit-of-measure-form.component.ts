...
export interface UnitOfMeasureFormDialogData {
  mode: FormMode;
  item?: UnitOfMeasure;
}
...
export class UnitOfMeasureFormComponent implements OnInit {
  ...
  private readonly data = inject<UnitOfMeasureFormDialogData | null>(MAT_DIALOG_DATA, { optional: true });
  ...
  get title(): string {
    return this.mode === 'create' ? 'New Unit of Measure' : 'Edit Unit of Measure';
  }
  ...
  save(): void {
    ...
    this.saving = true;
    const value = this.form.getRawValue() as CreateUnitOfMeasure;
    ...
    request$.subscribe({
      next: () => {
        ...
        this.notification.success(
          this.mode === 'create' ? 'Unit of Measure created.' : 'Unit of Measure updated.'
        );
        this.close(true);
      },
      error: () => {
