...
import { UnitOfMeasure } from '../../../Shared/Model/-unit-of-measure.model';
...
@Component({
  selector: 'app-view-unit-of-measure',
  standalone: true,
  imports: [MATERIAL_IMPORTS],
  templateUrl: './unit-of-measure.component.html',
  styleUrl: './unit-of-measure.component.css',
})
export class ViewUnitOfMeasureComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<UnitOfMeasure[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/inventory/unit-of-measure/new']);
   }

   onEdit(row: UnitOfMeasure): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/inventory/unit-of-measure', row.id, 'edit']);
   }

   onDelete(row: UnitOfMeasure): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Unit of Measure', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Unit of Measure removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: UnitOfMeasure): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: UnitOfMeasure }): void {
    this.dialog
      .open(UnitOfMeasureFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
