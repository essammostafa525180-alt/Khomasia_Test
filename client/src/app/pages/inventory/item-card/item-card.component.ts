...
import { InventoryItemFormComponent } from './item-card-form.component';
import { ConfirmDialogComponent } from '../../../Shared/Components/confirm-dialog/confirm-dialog.component';
import { MATERIAL_IMPORTS } from '../../../Shared/materail-imports';

@Component({
  selector: 'app-view-item-card',
  standalone: true,
  imports: [MATERIAL_IMPORTS],
  templateUrl: './item-card.component.html',
  styleUrl: './item-card.component.css',
})
export class ViewItemCardComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select', 'id', 'itemNumber', 'name', 'nameAr', 'totalQuantity', 'rfid', 'englishDescription', 'actions'];
  ...
  constructor(
    public service: ItemCardService,
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<InventoryItem[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/inventory/item-card/new']);
   }
   ...
   onEdit(row: InventoryItem): void {
     ...
     this.router.navigate(['/inventory/item-card', row.id, 'edit']);
   }
   ...
   onDelete(row: InventoryItem): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Item Card', message: 'Delete "' + (row.itemNumber ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Item Card removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: InventoryItem }): void {
    this.dialog
      .open(InventoryItemFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
