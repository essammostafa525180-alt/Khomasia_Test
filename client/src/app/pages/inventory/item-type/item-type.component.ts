...
import { ItemType } from '../../../Shared/Model/-item-type.model';
...
@Component({
  selector: 'app-view-item-type',
  standalone: true,
  imports: [MATERIAL_IMPORTS],
  templateUrl: './item-type.component.html',
  styleUrl: './item-type.component.css',
})
export class ViewItemTypeComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<ItemType[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/inventory/item-type/new']);
   }

   onEdit(row: ItemType): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/inventory/item-type', row.id, 'edit']);
   }

   onDelete(row: ItemType): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Item Type', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Item Type removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: ItemType): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: ItemType }): void {
    this.dialog
      .open(ItemTypeFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
