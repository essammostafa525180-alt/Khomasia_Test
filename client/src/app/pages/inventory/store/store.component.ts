...
export class ViewStoreComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Store[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/inventory/store/new']);
   }
   ...
   onEdit(row: Store): void {
     ...
     this.router.navigate(['/inventory/store', row.id, 'edit']);
   }
   ...
   onDelete(row: Store): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Store', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Store removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Store }): void {
    this.dialog
      .open(StoreFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
