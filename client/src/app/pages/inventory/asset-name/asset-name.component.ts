...
import { AssetFormComponent } from './asset-name-form.component';
import { ConfirmDialogComponent } from '../../../Shared/Components/confirm-dialog/confirm-dialog.component';
import { MATERIAL_IMPORTS } from '../../../Shared/materail-imports';

@Component({
  selector: 'app-view-asset-name',
  standalone: true,
  imports: [MATERIAL_IMPORTS],
  templateUrl: './asset-name.component.html',
  styleUrl: './asset-name.component.css',
})
export class ViewAssetNameComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select', 'id', 'code', 'name', 'nameAr', 'equipmentLocationCode', 'functionalCode', 'quantity', 'actions'];
  ...
  exportColumns: ExportColumn[] = [
    ...
    { key: 'equipmentLocationCode', label: 'Equipment Location Code' },
    { key: 'functionalCode', label: 'Functional Code' },
    { key: 'quantity', label: 'Quantity' },
  ];

  constructor(
    public service: AssetNameService,
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Asset[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/inventory/asset-name/new']);
   }
   ...
   onEdit(row: Asset): void {
     ...
     this.router.navigate(['/inventory/asset-name', row.id, 'edit']);
   }
   ...
   onDelete(row: Asset): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Asset Name', message: 'Delete "' + (row.code ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Asset Name removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Asset }): void {
    this.dialog
      .open(AssetFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
