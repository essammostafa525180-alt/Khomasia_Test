...
import { Shelf } from '../../../Shared/Model/-shelf.model';
...
export class ViewShelfComponent implements AfterViewInit {
  ...
  selection = new SelectionModel<Shelf>(true, []);
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Shelf[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/inventory/shelf/new']);
   }

   onEdit(row: Shelf): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/inventory/shelf', row.id, 'edit']);
   }

   onDelete(row: Shelf): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Shelf', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Shelf removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: Shelf): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: Shelf }): void {
    this.dialog
      .open(ShelfFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
