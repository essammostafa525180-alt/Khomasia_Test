...
export class ViewLocationComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/inventory/location/new']);
   }
   ...
   onEdit(row: Location): void {
     ...
     this.router.navigate(['/inventory/location', row.id, 'edit']);
   }
   ...
   onDelete(row: Location): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Location', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Location removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Location }): void {
    this.dialog
      .open(LocationFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
