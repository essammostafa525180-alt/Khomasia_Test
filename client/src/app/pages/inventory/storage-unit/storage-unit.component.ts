...
import { StorageUnit } from '../../../Shared/Model/-storage-unit.model';
...
@Component({
  selector: 'app-view-storage-unit',
  standalone: true,
  imports: [MATERIAL_IMPORTS],
  templateUrl: './storage-unit.component.html',
  styleUrl: './storage-unit.component.css',
})
export class ViewStorageUnitComponent implements AfterViewInit {
  ...
  dataSource = new MatTableDataSource<StorageUnit>([]);
  selection = new SelectionModel<StorageUnit>(true, []);
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<StorageUnit[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/inventory/storage-unit/new']);
   }

   onEdit(row: StorageUnit): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/inventory/storage-unit', row.id, 'edit']);
   }

   onDelete(row: StorageUnit): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Storage Unit', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Storage Unit removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: StorageUnit): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: StorageUnit }): void {
    this.dialog
      .open(StorageUnitFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
