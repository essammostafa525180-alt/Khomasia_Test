...
import { VendorOrderReceiveFormComponent } from './grn-form.component';
...
export class ViewGrnComponent implements AfterViewInit {
   ...
   onDelete(row: VendorOrderReceive): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete GRN', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('GRN removed (soft delete).');
             this.loadData();
           },
         });
       });
