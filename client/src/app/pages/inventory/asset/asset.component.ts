...
import { AssetEntityFormComponent } from './asset-form.component';
...
export class ViewAssetComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/inventory/asset/new']);
   }
   ...
   onEdit(row: Asset): void {
     ...
     this.router.navigate(['/inventory/asset', row.id, 'edit']);
   }
   ...
   onDelete(row: Asset): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Asset', message: 'Delete "' + (row.code ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Asset removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Asset }): void {
    this.dialog
      .open(AssetEntityFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
