...
import { Ou } from '../../../Shared/Model/-ou.model';
...
import { OuFormComponent } from './ou-form.component';
import { ConfirmDialogComponent } from '../../../Shared/Components/confirm-dialog/confirm-dialog.component';
import { MATERIAL_IMPORTS } from '../../../Shared/materail-imports';

@Component({
  selector: 'app-view-ou',
  standalone: true,
  imports: [MATERIAL_IMPORTS],
  templateUrl: './ou.component.html',
  styleUrl: './ou.component.css',
})
export class ViewOuComponent implements AfterViewInit {
  ...
  dataSource = new MatTableDataSource<Ou>([]);
  selection = new SelectionModel<Ou>(true, []);
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Ou[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/administration/ou/new']);
   }
   ...
   onEdit(row: Ou): void {
     ...
     this.router.navigate(['/administration/ou', row.id, 'edit']);
   }

   onDelete(row: Ou): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Ou', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Ou removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: Ou): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: Ou }): void {
    this.dialog
      .open(OuFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
