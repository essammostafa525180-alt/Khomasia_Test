...
export class ViewLanguageComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Language[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/administration/language/new']);
   }
   ...
   onEdit(row: Language): void {
     ...
     this.router.navigate(['/administration/language', row.id, 'edit']);
   }
   ...
   onDelete(row: Language): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Language', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Language removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Language }): void {
    this.dialog
      .open(LanguageFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
