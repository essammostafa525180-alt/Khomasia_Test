...
import { ContactType } from '../../../Shared/Model/-contact-type.model';
...
@Component({
  selector: 'app-view-contact-type',
  standalone: true,
  imports: [MATERIAL_IMPORTS],
  templateUrl: './contact-type.component.html',
  styleUrl: './contact-type.component.css',
})
export class ViewContactTypeComponent implements AfterViewInit {
  ...
  selection = new SelectionModel<ContactType>(true, []);
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<ContactType[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/administration/contact-type/new']);
   }

   onEdit(row: ContactType): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/administration/contact-type', row.id, 'edit']);
   }

   onDelete(row: ContactType): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Contact Type', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Contact Type removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: ContactType): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: ContactType }): void {
    this.dialog
      .open(ContactTypeFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
