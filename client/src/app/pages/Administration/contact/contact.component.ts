...
export class ViewContactComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select', 'id', 'contactValue', 'contactTypeId', 'updatedOn', 'actions'];

  exportColumns: ExportColumn[] = [
    { key: 'id', label: 'ID' },
    { key: 'contactValue', label: 'Contact Value' },
    { key: 'contactTypeId', label: 'Contact Type' },
  ...
  ngAfterViewInit(): void {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/administration/contact/new']);
   }
   ...
   onEdit(row: Contact): void {
     ...
     this.router.navigate(['/administration/contact', row.id, 'edit']);
   }
   ...
   onDelete(row: Contact): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Contact', message: 'Delete "' + (row.contactValue ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Contact removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Contact }): void {
    this.dialog
      .open(ContactFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
