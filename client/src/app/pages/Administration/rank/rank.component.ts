...
import { Rank } from '../../../Shared/Model/-rank.model';
...
export class ViewRankComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/administration/rank/new']);
   }

   onEdit(row: Rank): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/administration/rank', row.id, 'edit']);
   }

   onDelete(row: Rank): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Rank', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Rank removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Rank }): void {
    this.dialog
      .open(RankFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
