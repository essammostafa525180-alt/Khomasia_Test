...
export class ViewNotificationComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select', 'id', 'to', 'cc', 'bcc', 'phoneNumber', 'subject', 'body', 'actions'];
  ...
  constructor(
    ...
    private notification: CoreNotificationService,
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Notification[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/administration/notification/new']);
   }
   ...
   onEdit(row: Notification): void {
     ...
     this.router.navigate(['/administration/notification', row.id, 'edit']);
   }
   ...
   onDelete(row: Notification): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Notification', message: 'Delete "' + (row.to ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Notification removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Notification }): void {
    this.dialog
      .open(NotificationFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
