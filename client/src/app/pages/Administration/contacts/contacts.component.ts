...
export class ViewContactsComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Contacts[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/administration/contacts/new']);
   }
   ...
   onEdit(row: Contacts): void {
     ...
     this.router.navigate(['/administration/contacts', row.id, 'edit']);
   }
   ...
   onDelete(row: Contacts): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Contacts', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Contacts removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Contacts }): void {
    this.dialog
      .open(ContactsFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
