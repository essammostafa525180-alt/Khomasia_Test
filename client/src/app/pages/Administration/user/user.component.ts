...
export class ViewUserComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select', 'id', 'updatedOn', 'code', 'name', 'userId', 'password', 'email', 'actions'];
  ...
  ngAfterViewInit(): void {
   ...
   onEdit(row: User): void {
     ...
     this.router.navigate(['/administration/user', row.id, 'edit']);
   }
   ...
   onDelete(row: User): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete User', message: 'Delete "' + (row.code ?? row.id) + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('User removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: User }): void {
    this.dialog
      .open(UserFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
