...
import { VendorOrder } from '../../../Shared/Model/-vendor-order.model';
...
export class ViewVendorOrderComponent implements AfterViewInit {
  ...
  selection = new SelectionModel<VendorOrder>(true, []);
  ...
  constructor(
    public service: VendorOrderService,
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<VendorOrder[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/procurement/vendor-order/new']);
   }

   onEdit(row: VendorOrder): void {
     if (this.viewMode.isDialog()) {
       this.openForm({ mode: 'edit', item: row });
       return;
     }
     this.router.navigate(['/procurement/vendor-order', row.id, 'edit']);
   }

   onDelete(row: VendorOrder): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Vendor Order', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Vendor Order removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  checkboxLabel(row?: VendorOrder): string {
    if (!row) return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
    return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row`;
  }

  private openForm(data: { mode: 'create' | 'edit'; item?: VendorOrder }): void {
    this.dialog
      .open(VendorOrderFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
