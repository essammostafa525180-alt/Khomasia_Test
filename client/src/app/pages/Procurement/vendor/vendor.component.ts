...
export class ViewVendorComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
   ...
   loadData(): void {
     this.service.getAll<Vendor[]>().subscribe((data) => {
   ...
   onNew(): void {
     ...
     this.router.navigate(['/procurement/vendor/new']);
   }
   ...
   onEdit(row: Vendor): void {
     ...
     this.router.navigate(['/procurement/vendor', row.id, 'edit']);
   }
   ...
   onDelete(row: Vendor): void {
     this.dialog
       .open(ConfirmDialogComponent, {
         width: '400px',
         data: { title: 'Delete Vendor', message: 'Delete "' + row.id + '"? This action sets the record inactive (soft delete).' },
       })
       .afterClosed()
       .subscribe((confirmed) => {
         if (!confirmed) return;
         this.service.softDelete(row.id).subscribe({
           next: () => {
             this.notification.success('Vendor removed (soft delete).');
             this.loadData();
           },
         });
       });
  ...
  private openForm(data: { mode: 'create' | 'edit'; item?: Vendor }): void {
    this.dialog
      .open(VendorFormComponent, { width: '720px', panelClass: 'crud-dialog', data })
      .afterClosed()
      .subscribe((saved) => {
