using Application.Abstractions;

namespace Application.CQRS.CostCenter.Commands;

public class DeleteCostCenterCommand : ICommand<Result>
{
    public int Id { get; set; }
}
internal class DeleteCostCenterCommandHandler : ICommandHandler<DeleteCostCenterCommand, Result>
{
    private readonly IUnitOfWork _unitOfWork;

    public DeleteCostCenterCommandHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<Result> Handle(DeleteCostCenterCommand request, CancellationToken cancellationToken)
    {
        var entity = await _unitOfWork.CostCenterRepository.GetByIdAsync(request.Id);

        if (entity is null || entity.IsDeleted)
            return Result.Failure(Errors.CostCenterNotFound);

        _unitOfWork.CostCenterRepository.SoftDelete(entity);

        var result = await _unitOfWork.SaveChangesAsync(cancellationToken);

        return result > 0 ? Result.Success() : Result.Failure(Errors.CostCenterNotDeleted);
    }
}