...
public class CreateInventroyItemRequestWithdrawCommand : ICommand<Result<int>>
{
...
internal class CreateInventroyItemRequestWithdrawCommandHandler : ICommandHandler<CreateInventroyItemRequestWithdrawCommand, Result<int>>
{
    private readonly IUnitOfWork _unitOfWork;

    public CreateInventroyItemRequestWithdrawCommandHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<Result<int>> Handle(CreateInventroyItemRequestWithdrawCommand request, CancellationToken cancellationToken)
    {
        var entity = Domain.Aggregates.RequestAggregate.InventroyItemRequestWithdraw.Create(request.ItemTypeFk, request.RequestNo, request.RequestDate, request.DescriptionEn, request.DescriptionAr, request.IsApproved, request.RequestedByFk, request.RequestedBy, request.AssignedToUserFk, request.ItemRequestStatusFk, request.WorkOrderNo, request.StoreFk, request.SentCount, request.Axsynced, request.ProjectFk, request.Oufk, request.ItemNeededDate, request.ScopeFk, request.CompanyFk, request.ServiceMainCategoryFk, request.SiteManagerApproval, request.SiteManagerApprovalUserId, request.SiteManagerApprovalDateTime, request.WarehouseManagerApprovalUserId, request.WarehouseManagerApprovalDateTime, request.LocationFk, request.InventoryItemBudgetFk, request.SourceTypeId, request.EntityId, request.EntityFormula, request.ReceivedFk, request.VehicleFk, request.LineFk, request.SourceEntity, request.SourceId, request.SectorFk, request.CostCenterFk, request.CustomerFk, request.FactoryFk, request.FactoryLineFk, request.IsActive);

        await _unitOfWork.InventroyItemRequestWithdrawRepository.AddAsync(entity);
        var result = await _unitOfWork.SaveChangesAsync(cancellationToken);

        return result > 0
            ? Result<int>.Success(entity.Id)
            : Result<int>.Failure(Errors.InventroyItemRequestWithdrawNotInserted);
    }
}