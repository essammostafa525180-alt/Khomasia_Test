...
internal class DeleteEmployeeCommandHandler : ICommandHandler<DeleteEmployeeCommand, Result>
{
    ...
    public async Task<Result> Handle(DeleteEmployeeCommand request, CancellationToken cancellationToken)
    {
        ...
        _unitOfWork.EmployeeRepository.SoftDelete(entity);

        var result = await _unitOfWork.SaveChangesAsync(cancellationToken);

        return result > 0 ? Result.Success() : Result.Failure(Errors.EmployeeNotDeleted);
    }
}