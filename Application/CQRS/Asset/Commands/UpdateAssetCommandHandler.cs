...
public class UpdateAssetCommand : ICommand<Result>
{
...
internal class UpdateAssetCommandHandler : ICommandHandler<UpdateAssetCommand, Result>
{
    ...
    public async Task<Result> Handle(UpdateAssetCommand request, CancellationToken cancellationToken)
    {
        ...
        entity.Update(request.AssetGroupFk, request.AssetTypeFk, request.ToolsTypeFk, request.Code, request.Name, request.NameAr, request.ZoneFk, request.EquipmentCodeFk, request.EquipmentLocationCode, request.FunctionalCode, request.Quantity, request.CostPerHour, request.CurrencyFk, request.WarrantyStatusFk, request.Rfid, request.Remarks, request.PossessionTypeFk, request.OperationDate, request.IsOperational, request.InsuranceVendorFk, request.PolicyNumber, request.PolicyDate, request.PolicyExpiryDate, request.PolicyAmount, request.ManufactureFk, request.Model, request.ModelYearFk, request.SerialNumber, request.GuaranteeExpiryDate, request.TechnicalInformation, request.Axsynced, request.ProjectFk, request.AssetStatusFk, request.PurchasePrice, request.PurchaseDate, request.CheckDate, request.LifeTime, request.DepreciationRate, request.PlannedDepreciationDate, request.ActualDepreciationDate, request.Oufk, request.IsActive);

        var result = await _unitOfWork.SaveChangesAsync(cancellationToken);

        return result > 0 ? Result.Success() : Result.Failure(Errors.AssetNotUpdated);
    }
}